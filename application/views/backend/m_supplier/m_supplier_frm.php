        ...
        <div id="div1" class="table-responsive">
            <table ng-table="tableList" show-filter="false" class="table table-condensed table-bordered table-hover"
                ...
                <tr ng-repeat="(k,v) in $data" class="pointer" ng-click="read(v.id)">
                    ...
                    <td title="'Jenis Supplier'" filter="{jenis: 'text'}" sortable="'jenis'">{{v.jenis}}</td>
                </tr>
            </table>
        </div>
    </div>
    ...
    <div class="ibox-content frmEntry">
        <div class="row">
            ...
            <div class="col-sm-4">
                ...
                <label title="jenis">Jenis Supplier</label>
                <select ng-model="h.jenis" class="form-control input-sm">
                    <option ng-repeat="v in [['UMUM','UMUM']]" ng-value="v[0]">{{v[1]}}</option>
                </select>
            </div>
        </div>
    </div>
...
app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', 'FileUploader', function($scope, $http,
    ...
    SfService.setUrl("<?=base_url()?>m_supplier");
    $scope.f = {
        crud: 'c',
        tab: 'list',
        pk: 'id'
