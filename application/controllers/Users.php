...
class Users extends CI_Controller
{
    ...
    {
        $data=[
            'users'=>$this->db->get_where('users',['id_user'=>getSession('id_user')])->row(),
            'action' => site_url('users/updatePassword'),
            'content' => 'backend/users/change_password.php',
    ];
        $this->load->view(layout(), $data);
    }
    ...
    public function updatePassword(){
        ...
        $ulangi=$this->input->post('ulangi_password_baru');
        if(md5($lama)!=$this->db->get_where('users',['id_user'=>getSession('id_user')])->row()->password){
            $this->session->set_flashdata('message', 'Password lama tidak tepat');
            redirect(site_url('users/changePassword'));
        }

        if($baru=="" || $ulangi==""){
            $this->session->set_flashdata('message', 'Perbaiki password baru dan ulangi password harus diisi');
            redirect(site_url('users/changePassword'));
        }
        ...
        $this->db->where('id_user',getSession('id_user'));
        $this->db->update('users',['password'=>md5($baru)]);
        $this->session->set_flashdata('message', 'Berhasil rubah password, silakan login ulang');
        redirect(site_url('users/changePassword'));
    }
