...
class M_konversi extends CI_Controller
{
    private $m;
    function __construct()
    {
        ...
        $this->m = new M_konversi_model();
    }

    public function index()
    {
        $data = array(
            'content' => "backend/m_konversi/m_konversi_frm",
        );
    ...
    private function queryList(&$total, &$current, $page, $limit, $q, $arr_where)
    {
        $total = $this->db->from($this->m->table . " aa")
            ...
            ->where($arr_where)
            ->group_end()
            ->join('m_barang bb', 'aa.id_barang=bb.id')
            ->count_all_results();
        $current = $this->db->select("aa.*,bb.nama as nama_barang")
    ...
    public function prin()
    {
        ...
        $data = array(
            'h' => $data->row(),
            'content' => 'backend/m_konversi/m_konversi_print',
        );
