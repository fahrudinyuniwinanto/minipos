...
class Tagihan_bpjs extends CI_Controller
{
    private $m;
    public function __construct()
    {
        parent::__construct();
        is_logged();
        $this->load->model('Tagihan_bpjs_model');
        $this->m = new Tagihan_bpjs_model();
    }

    public function tgl()
    {
        echo @date_format(date_create("05/01/2022
        "), "Y-m-d");
    }
    ...
    {
        $data = array(
            'content' => "backend/tagihan_bpjs/tagihan_bpjs_frm",
        );
    ...
    {
        $total = $this->db->from($this->m->table." as aa")
            ->like('aa.id', $q)
            ->group_start()
            ->where($arr_where)
            ->group_end()
            ->count_all_results();
        $current = $this->db->from($this->m->table." as aa")
        ->select("aa.*,bb.nama")
            ->like('aa.id', $q)
            ->group_start()
            ->where($arr_where)
            ->group_end()
            ->limit($limit, ($page * $limit) - $limit)->order_by("aa.".$this->m->id, $this->m->order)
            ->join('m_barang as bb','bb.id=aa.id_barang')
            ->get();
    }
    ...
    public function prin()
    {
        ...
        $data = array(
            'h'       => $data->row(),
            'content' => 'backend/tagihan_bpjs/tagihan_bpjs_print',
        );
